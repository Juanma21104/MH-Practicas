...
class AtomicOrbitalSearch:
    ...
    def _set_current_bs_and_le(self):
        ...
        #Eliminar las peores soluciones hasta quedarnos con n_electrons
        self.energy_levels = self.energy_levels[:self.n_electrons]
        self.electrons = self.electrons[:self.n_electrons]
    ...
    def magnetic_perturbation(self, electron):
        ...
        """
        return electron + np.random.uniform(-0.5, 0.5, size=self.dim)
    ...
    def optimize(self):
        ...
        for i in range(self.max_iter):
            ...
            for layer in range(1, n + 1):
                ...
                if layers[layer-1] == layers[layer]:
                    continue
                ...
                for electron_index in range(layers[layer-1], layers[layer]):
                    electron = self.electrons[electron_index].copy()
                    phi = np.random.rand()

                    if phi <= self.pr:
                        if self.energy_levels[electron_index] >= be:
                            #Emision foton
                            new_electron = self.photon_emission(electron, layer)
                        else:
                            #Absorcion foton
                            new_electron = self.photon_absorption(electron, le, bs)
                    else:
                        #Perturbacion aleatoria
                        new_electron = self.magnetic_perturbation(electron)

                    #Manejo de los limites de las variables de decision
                    new_electron = np.maximum(new_electron, self.lb)
                    new_electron = np.minimum(new_electron, self.ub)
                    new_electrons.append(new_electron)

                    #Actualizacion del nivel de energia de la solucion
                    new_energy_levels.append(self.obj_func(new_electron))
                
            #Append de las nuevas soluciones
            self.electrons = np.append(self.electrons, new_electrons, axis=0)
            self.energy_levels = np.append(self.energy_levels, new_energy_levels)
...
def plot_results_dynamic(optimizer_instance):
    ...
    for i, iter_idx in enumerate(snapshots):
        ...
        if iter_idx == 0:
            ...
            if range_x < 1e-65: range_x = 1e-55
            if range_y < 1e-65: range_y = 1e-55
